    ...
    let n = 10
        ...
        @testset "trsv" begin
            ...
            @testset "Vector and SubVector" for x in (rand(elty, n), view(rand(elty,2n),1:2:2n))
                ...
                @test_throws DimensionMismatch BLAS.trsv('U','N','N',A,Vector{elty}(n+1))
            end
        end
        @testset "ger, her, syr" for x in (rand(elty, n), view(rand(elty,2n), 1:2:2n)),
            ...
            @test_throws DimensionMismatch BLAS.ger!(α,Vector{elty}(n+1),y,copy(A))

            A = rand(elty,n,n)
            A = A + A.'
            @test issymmetric(A)
            @test triu(BLAS.syr!('U',α,x,copy(A))) ≈ triu(A + α*x*x.')
            @test_throws DimensionMismatch BLAS.syr!('U',α,Vector{elty}(n+1),copy(A))
            ...
            if elty <: Complex
                ...
                @test_throws DimensionMismatch BLAS.her!('U',α,Vector{elty}(n+1),copy(A))
            end
        end
        @testset "copy" begin
            x1 = convert(Vector{elty}, randn(n))
        ...
        @testset "symmetric/Hermitian multiplication" begin
            ...
            @testset "symv and hemv" begin
                ...
                offsizevec, offsizemat = Array{elty}.((n+1, (n,n+1)))
        ...
        @testset "trmm error throwing" begin
            Cnn, Cmn, Cnm = Matrix{elty}.(((n,n), (n+1,n), (n,n+1)))
